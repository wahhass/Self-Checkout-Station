...
public class SystemTestingMultipleItemsNonMember {
	...
	//Tests for a non-member scanning multiple items and paying with cash and coin
		@ Test
		public void NonMemberMultipleItemsCashAndCoin() throws Exception {
			cart.clearCart();
			System.out.println();
			...
			notePayment.payNote(twentyDollar);
							
			Assert.assertEquals(20, notePayment.getTotalCashInserted()); //cash inserted properly		
				
			//calculate change
			BigDecimal value = cart.getCumPrice().subtract(new BigDecimal(20.2));
	...
	//Tests for a non-member scanning multiple items and paying with a debit card
	@ Test
	public void NonMemberMultipleItemsDebit() throws Exception {
		cart.clearCart();
		System.out.println();
	...
	//Tests for a non-member scanning multiple items and paying with a gift card
	@ Test
	public void NonMemberMultipleItemsGiftCard() throws Exception {
		cart.clearCart();
		System.out.println();
	...
	//Tests for a non-member purchasing multiple items and paying with a debit card using their own bags
	@ Test
	public void OwnBagMultipleItemsDebit() throws Exception {
		cart.clearCart();
		System.out.println();
	...
	//Tests for a non-member scanning multiple items and paying with a gift card using their own bags
	@ Test
	public void OwnBagMultipleItemsGiftCard() throws Exception {
		cart.clearCart();
		System.out.println();
	...
	//Tests for a non-member scanning multiple items and paying with a credit card using their own bags
	@ Test
	public void OwnBagMultipleItemsCredit() throws Exception {
		cart.clearCart();
		System.out.println();
		...
		// Enter PLU items
		plu.enterPLUItem(new PriceLookupCode("4053")); //enter a lemon

		try {
			// Bag item
			cart.addPLUCodedProductToCart(plu.PLU_PRODUCT_DATABASE.get(new PriceLookupCode("4053")), null);
		} catch (Exception e) {
			e.printStackTrace();
		}
    ...
    public
    void
    NonMemberScanItemKeyItemBagCreditCard()
            throws
            Exception
    {
...
// Scan items
        ...
        plu.enterPLUItem(new
                PriceLookupCode("4205"));

        double
                productPrice = 4.30;

        double
                predictedPrice = productPrice * (plu.getPLUItemWeight() * 0.001);

        String
                formattedPrice = df.format(predictedPrice);
        ...
        double
                predictedPrice2 = productPrice2 * (plu.getPLUItemWeight() * 0.001);

        PLUCodedItem
                itemForBag2 = new
                PLUCodedItem(new
                PriceLookupCode("4205"), plu.getPLUItemWeight());

        String
                formattedPrice2 = df.format(predictedPrice2);
        ...
        double
                predictedPrice3 = productPrice3 * (plu.getPLUItemWeight() * 0.001);

        String
                formattedPrice3 = df.format(predictedPrice3);
        ...
        Assert.assertEquals(
                "Milk: $4.5\nMilk: $4.5\nMilk: $4.5\nMeat: $10\nMeat: $10\nCheese: $5\nCheese: $5\nApples: $" + formattedPrice + "\nApples: $" + formattedPrice2 + "\nApples: $" + formattedPrice3 + "\n",
                scanner.getProductArray());
...
// Bag item
            ...
            baggingArea.AddItem(milkItem3);
            baggingArea.AddItem(meatItem);
            baggingArea.AddItem(meatItem2);
            baggingArea.AddItem(cheeseItem);
            baggingArea.AddItem(cheeseItem2);
            Assert.assertEquals(baggingArea.predictedWeightOnBagging, 43.5, 0);
            baggingArea.AddItem(itemForBag);
            baggingArea.AddItem(itemForBag2);
            baggingArea.AddItem(itemForBag3);
        } catch (Exception e) {
// TODO Auto-generated catch block
            e.printStackTrace();
        }

// Select number of bagsused

        baggingArea.addNumberPlasticBagsUsed(4);

        Assert.assertEquals(43.5 + predictedPrice + predictedPrice2 + predictedPrice3 + 0.4, cart.getCumPrice().doubleValue(),
                0.1);
